...
this["Fliplet"]["Widget"]["Templates"]["templates.components.timer"] = Handlebars.template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    return "<div v-if=\"description\" class=\"help-block description\">{{ description }}</div>\n<template>\n  <div class=\"form-group fl-timer\" ref=\"timer\" :aria-label=\"T(`widgets.form.timer.types.${type}`)\">\n    <input type=\"text\" class=\"form-control\" readonly=\"true\" tabindex=\"0\" v-model=\"value\" />\n    <button v-if=\"!readonly && timerStatus === 'paused'\" class=\"btn btn-primary\" v-on:click.prevent=\"timer.reset\">Reset</button>\n    <button v-if=\"!readonly && (timerStatus === 'initial' || timerStatus === 'paused')\" class=\"btn btn-primary\" v-on:click.prevent=\"timer.start\">Start</button>\n    <button v-if=\"!readonly && timerStatus === 'running'\" class=\"btn btn-primary\" v-on:click.prevent=\"timer.stop\">Stop</button>\n  </div>\n</template>\n<p class=\"text-danger\" v-if=\"$v.value.required === false && $v.value.$dirty\">\n  "
    + container.escapeExpression((helpers.T || (depth0 && depth0.T) || helpers.helperMissing).call(depth0 != null ? depth0 : (container.nullContext || {}),"widgets.form.errors.required",{"name":"T","hash":{},"data":data}))
    + "\n</p>\n";
},"useData":true});
